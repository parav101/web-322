...
// Company.updateOne({ companyName: "The Kwik-E-Mart" } // the first one will be updated
Company.updateMany({ employeeCount: 3 }
  , { $set: { employeeCount: 5 } }
)
.exec()
.then((msg) => {
...
Company.updateOne({ studentID: 105160179 } // the first one will be update
  , { $set: { program: "CPA" } }
)
.exec()
.then((msg) => {
    if(msg) {
        console.log("succesfully updated: ", msg);
    } 
    // exit the program after saving and finding
    process.exit();
})
